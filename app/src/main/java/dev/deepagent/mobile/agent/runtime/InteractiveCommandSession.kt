...
class InteractiveCommandSession(
    ...
    fun cancelActive(sessionId: String? = null): Boolean {
        val process = synchronized(processLock) {
            if (sessionId == null || activeSessionId == sessionId) activeProcess else null
        } ?: return false
        terminate(process)
        return true
    }
    ...
    private fun validate(request: InteractiveCommandRequest): Validation? {
        ...
        if (request.args.any { argument ->
                argument.isBlank() ||
                    argument.length > MAX_ARG_CHARS ||
                    argument.contains('\u0000') ||
                    argument.startsWith("/") ||
                    argument.split('/').any { it == ".." } ||
                    argument == "-c" ||
        ...
        val cwd = File(root, request.cwd).canonicalFile
        if (
